<x-admin-layout :title="'ویرایش ' . $config['label']">
    <div class="max-w-xl">
        <div class="bg-white rounded-2xl shadow-soft border border-surface-100 p-6">
            <form method="POST" action="{{ route('admin.lookup.update', [$type, $item->id]) }}" class="space-y-5">
                @csrf
                @method('PUT')

                <div>
                    <label for="name" class="block text-sm font-medium text-surface-700 mb-1">نام انگلیسی</label>
                    <input type="text" name="name" id="name" value="{{ old('name', $item->name) }}" class="form-input" required>
                    @error('name') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label for="name_fa" class="block text-sm font-medium text-surface-700 mb-1">نام فارسی</label>
                    <input type="text" name="name_fa" id="name_fa" value="{{ old('name_fa', $item->name_fa) }}" class="form-input" required>
                ...
                <div>
                    ...
                    <input type="text" name="slug" id="slug" value="{{ old('slug', $item->slug) }}" class="form-input" dir="ltr">
                ...
                @if(in_array('category', $config['fields']))
                    <div>
                        ...
                        <input type="text" name="category" id="category" value="{{ old('category', $item->category) }}" class="form-input">
