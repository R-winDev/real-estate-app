...
<a href="{{ route('properties.show', $property) }}" class="group card-hover overflow-hidden">
    ...
    <div class="relative bg-gradient-to-br from-brand-100 via-brand-50 to-surface-100 h-56 flex items-center justify-center overflow-hidden">
        ...
        @if($property->status)
            <span class="absolute top-3 right-3 px-3 py-1 rounded-full text-xs font-bold backdrop-blur-sm
                {{ $property->status?->slug === 'active' ? 'bg-green-500/90 text-white shadow-lg shadow-green-500/25' : ($property->status?->slug === 'sold' ? 'bg-amber-500/90 text-white shadow-lg shadow-amber-500/25' : 'bg-surface-500/90 text-white shadow-lg shadow-surface-500/25') }}">
    ...
    <div class="p-5">
        ...
        <div class="flex items-center gap-3 text-xs text-surface-500 mb-4">
            @if($property->area_total)
                <div class="flex items-center gap-1 bg-surface-50 px-2 py-1 rounded-lg">
            ...
            @endif
            @if($property->type)
                <span class="px-2 py-1 bg-brand-50 text-brand-700 rounded-lg text-xs font-medium">{{ $property->type->name_fa }}</span>
            @endif
        </div>
