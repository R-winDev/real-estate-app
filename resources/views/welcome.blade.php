<x-app-layout>
    ...
    <section class="relative bg-gradient-to-br from-brand-700 via-brand-800 to-surface-900 text-white overflow-hidden">
        ...
        <div class="relative container-wide section-padding pb-12 md:pb-20">
            <div class="max-w-3xl mb-10">
                ...
                <p class="text-base md:text-lg text-brand-100/70 leading-relaxed max-w-xl">
            ...
            <form action="{{ route('properties.index') }}" method="GET"
                ...
                <div class="flex flex-col md:flex-row gap-2">
                    <div class="flex-1 relative">
                        <svg class="absolute right-4 top-1/2 -translate-y-1/2 w-5 h-5 text-surface-400 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                        <input type="text" name="search" placeholder="جستجو در عنوان، آدرس یا توضیحات..."
                               class="w-full pr-12 pl-4 py-3.5 rounded-xl border-0 bg-surface-50 text-sm text-surface-800 placeholder:text-surface-400 focus:ring-2 focus:ring-brand-500/20 focus:bg-white transition-all duration-200">
                    </div>
                    <div class="flex gap-2">
                        <select name="type_id" class="px-4 py-3.5 rounded-xl border-0 bg-surface-50 text-sm text-surface-700 focus:ring-2 focus:ring-brand-500/20 transition-all duration-200 min-w-[140px]">
                            <option value="">همه انواع</option>
                            @foreach($propertyTypes as $type)
                                <option value="{{ $type->id }}">{{ $type->name_fa }}</option>
                            @endforeach
                        </select>
                        <select name="location_id" class="px-4 py-3.5 rounded-xl border-0 bg-surface-50 text-sm text-surface-700 focus:ring-2 focus:ring-brand-500/20 transition-all duration-200 min-w-[140px]">
                            <option value="">همه موقعیت‌ها</option>
                            @foreach($locations as $location)
                                <option value="{{ $location->id }}">{{ $location->name }}</option>
                            @endforeach
                        </select>
                        <button type="submit" class="bg-brand-600 hover:bg-brand-700 text-white px-6 py-3.5 rounded-xl font-semibold text-sm transition-all duration-200 flex items-center gap-2 shrink-0">
    ...
    <section class="bg-white border-b border-surface-100">
        <div class="container-wide py-5">
            <div class="flex items-center gap-4 overflow-x-auto pb-1 scrollbar-none">
                <span class="text-sm font-semibold text-surface-500 shrink-0">دسته‌بندی:</span>
                <a href="{{ route('properties.index') }}" class="shrink-0 px-4 py-2 rounded-full text-sm font-medium bg-surface-100 text-surface-600 hover:bg-brand-50 hover:text-brand-700 transition-all duration-200">
                    همه
                </a>
                @foreach($propertyTypes as $type)
                    <a href="{{ route('properties.index', ['type_id' => $type->id]) }}"
                       class="shrink-0 px-4 py-2 rounded-full text-sm font-medium bg-surface-100 text-surface-600 hover:bg-brand-50 hover:text-brand-700 transition-all duration-200">
                        {{ $type->name_fa }}
                    </a>
                @endforeach
            </div>
        ...
        <div class="container-wide">
            <div class="flex justify-between items-end mb-10">
                <div>
                    ...
                    <p class="text-surface-500 mt-2 text-sm">جدیدترین املاک ثبت شده در سامانه</p>
                </div>
    ...
    <section class="section-padding bg-white">
        <div class="container-wide">
            <div class="text-center mb-14">
                ...
                <p class="text-surface-500 mt-3 text-sm">امکانات ویژه برای تجربه بهتر خرید و فروش ملک</p>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="group text-center p-8 rounded-3xl bg-gradient-to-br from-surface-50 to-white border border-surface-100 hover:border-brand-200 hover:shadow-glow transition-all duration-300">
                    <div class="w-14 h-14 bg-gradient-to-br from-brand-500 to-brand-600 rounded-2xl flex items-center justify-center mx-auto mb-5 shadow-lg shadow-brand-500/20 group-hover:scale-110 transition-all duration-300">
                    ...
                    <p class="text-surface-500 leading-relaxed text-sm">با فیلترهای پیشرفته بر اساس موقعیت، قیمت، متراژ و امکانات، ملک دلخواه خود را سریع پیدا کنید.</p>
                </div>
                ...
                <div class="group text-center p-8 rounded-3xl bg-gradient-to-br from-surface-50 to-white border border-surface-100 hover:border-brand-200 hover:shadow-glow transition-all duration-300">
                    ...
                    <p class="text-surface-500 leading-relaxed text-sm">تمامی اطلاعات املاک توسط کارشناسان ما تایید شده و با اطمینان خاطر می‌توانید تصمیم بگیرید.</p>
                </div>
                ...
                <div class="group text-center p-8 rounded-3xl bg-gradient-to-br from-surface-50 to-white border border-surface-100 hover:border-brand-200 hover:shadow-glow transition-all duration-300">
                    ...
                    <p class="text-surface-500 leading-relaxed text-sm">ثبت و مدیریت ملک در کمتر از چند دقیقه با رابط کاربری ساده و کاربردی ما.</p>
                </div>
            </div>
        </div>
    </section>
    ...
    {{-- CTA --}}
    <section class="section-padding bg-gradient-to-br from-brand-600 via-brand-700 to-brand-800 text-white relative overflow-hidden">
        ...
        <div class="relative container-wide text-center">
            ...
            <p class="text-brand-100/70 mb-8 max-w-lg mx-auto">همین الان شروع به جستجو کنید یا ملک خود را ثبت نمایید.</p>
            <div class="flex flex-wrap justify-center gap-4">
                <a href="{{ route('properties.index') }}" class="bg-white text-brand-700 px-8 py-3.5 rounded-xl font-bold text-sm hover:bg-brand-50 transition-all duration-200 shadow-lg">
                    جستجوی املاک
                </a>
                @guest
