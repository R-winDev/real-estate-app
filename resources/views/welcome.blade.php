    ...
    <body class="font-sans antialiased bg-surface-50">
            ...
            <div class="container-wide">
                <div class="flex justify-between items-center h-16">
                    <div class="flex items-center gap-3">
                        ...
                        <span class="text-xl font-extrabold text-surface-900">املاک</span>
                    </div>

                    @if (Route::has('login'))
                        <nav class="flex items-center gap-4">
                            <a href="{{ route('properties.index') }}" class="text-sm font-medium text-surface-600 hover:text-surface-900 transition-colors duration-200">املاک</a>
                            @auth
                                <a href="{{ route('dashboard') }}" class="text-sm font-medium text-surface-600 hover:text-surface-900 transition-colors duration-200">داشبورد</a>
                            @else
                                <a href="{{ route('login') }}" class="text-sm font-medium text-surface-600 hover:text-surface-900 transition-colors duration-200">ورود</a>
                                @if (Route::has('register'))
                                    <a href="{{ route('register') }}" class="btn-primary text-sm">ثبت نام</a>
                                @endif
                            @endauth
                        </nav>
                    @endif
        ...
        <section class="relative bg-gradient-to-br from-brand-700 via-brand-800 to-brand-950 text-white overflow-hidden">
            ...
            <div class="relative container-wide section-padding">
                <div class="max-w-3xl">
                    <div class="inline-flex items-center gap-2 bg-white/10 backdrop-blur-sm rounded-full px-4 py-1.5 mb-6 border border-white/10">
                    ...
                    <div class="flex flex-wrap gap-4">
                        <a href="{{ route('properties.index') }}" class="inline-flex items-center gap-2 bg-white text-brand-800 px-8 py-4 rounded-2xl font-bold text-base hover:bg-brand-50 transition-all duration-300 shadow-xl hover:shadow-2xl hover:-translate-y-0.5">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                            مشاهده املاک
                        </a>
                        @auth
                            <a href="{{ route('properties.create') }}" class="inline-flex items-center gap-2 bg-white/10 backdrop-blur-sm text-white border border-white/20 px-8 py-4 rounded-2xl font-bold text-base hover:bg-white/20 transition-all duration-300">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
                                ثبت ملک جدید
                            </a>
                        @else
                            <a href="{{ route('register') }}" class="inline-flex items-center gap-2 bg-white/10 backdrop-blur-sm text-white border border-white/20 px-8 py-4 rounded-2xl font-bold text-base hover:bg-white/20 transition-all duration-300">
                                شروع رایگان
                            </a>
                        @endauth
                    </div>
        ...
        <section class="section-padding bg-white">
            <div class="container-wide">
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
                    <div class="glass rounded-2xl p-6 text-center hover:shadow-glow transition-all duration-300 group">
        ...
        <section class="section-padding bg-white">
            <div class="container-wide">
                <div class="text-center mb-12">
                    ...
                    <p class="text-surface-500 mt-2">امکانات ویژه برای تجربه بهتر خرید و فروش ملک</p>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="group text-center p-8 rounded-3xl bg-gradient-to-br from-surface-50 to-white border border-surface-100 hover:border-brand-200 hover:shadow-glow transition-all duration-300">
                        <div class="w-16 h-16 bg-gradient-to-br from-brand-500 to-brand-600 rounded-2xl flex items-center justify-center mx-auto mb-5 shadow-lg shadow-brand-500/25 group-hover:scale-110 transition-transform duration-300">
        ...
        <!-- Footer -->
        <footer class="bg-surface-900 text-surface-300 py-12">
            <div class="container-wide">
