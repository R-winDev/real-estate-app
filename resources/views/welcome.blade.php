    ...
    <section class="relative bg-gradient-to-br from-primary-700 via-primary-800 to-primary-900 text-white overflow-hidden">
        ...
        <div class="relative container-wide section-padding pb-12 md:pb-20">
            ...
            <form action="{{ route('properties.index') }}" method="GET"
                  class="bg-white rounded-2xl p-2 shadow-elevated-lg max-w-4xl" x-data="{ heroListingType: 'sale' }">
                <div class="flex flex-col md:flex-row gap-2">
                    <div class="flex rounded-xl overflow-hidden border border-neutral-200">
                        <button type="button" @click="heroListingType = 'sale'"
                            :class="heroListingType === 'sale' ? 'bg-primary-500 text-white' : 'bg-neutral-50 text-neutral-600 hover:bg-neutral-100'"
                            class="px-4 py-3.5 text-sm font-semibold transition-all duration-200 border-0">
                            خرید
                        </button>
                        <button type="button" @click="heroListingType = 'rental'"
                            :class="heroListingType === 'rental' ? 'bg-primary-500 text-white' : 'bg-neutral-50 text-neutral-600 hover:bg-neutral-100'"
                            class="px-4 py-3.5 text-sm font-semibold transition-all duration-200 border-0">
                            اجاره
                        </button>
                    </div>
                    <input type="hidden" name="listing_type" :value="heroListingType">
