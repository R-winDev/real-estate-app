<x-admin-layout title="ثبت ملک جدید">
    <div class="max-w-4xl mx-auto" x-data="propertyForm()" {{-- Page Header --}}
            ...
            <div class="rounded-xl border border-neutral-200 bg-white overflow-hidden">
                ...
                <div class="p-6">
                    ...
                    <div x-show="listingType === 'rental'" x-transition>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
                            <x-form-input name="deposit_amount" type="number" label="مبلغ رهن (تومان)" />
                            <x-form-input name="rent_amount" type="number" label="مبلغ اجاره ماهانه (تومان)" />
                            <x-form-select name="owner_id" label="صاحب ملک" :options="$users" />
                        </div>
                    </div>
    ...
    <script>
        function propertyForm() {
            return {
                listingType: '{{ old('listing_type', 'sale') }}'
            }
        }
    </script>
</x-admin-layout>
